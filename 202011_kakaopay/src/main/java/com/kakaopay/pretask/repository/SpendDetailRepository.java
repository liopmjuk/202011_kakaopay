...
public interface SpendDetailRepository extends JpaRepository<SpendMoneyDetail, SpendKey> {
	...
	public static final String COUNT_BY_TOKEN_AND_USERID = "select count(*) from spend_money_detail d where d.token = :token and d.userId = :userId";
	...
	public int countUserReceived(@Param("token") String token, @Param("userId") int userId);
}
